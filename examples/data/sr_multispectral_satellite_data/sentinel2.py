...
class S2_Dataloader(DataLoader):
    def __init__(self, dataset, suffix ,**kwargs):
        ...
        def collate_fn( batch):
            # Use the custom_collate_fn and pass the dataset
...
class SRDataset_perimg_lightload(Dataset):
    def __init__(self, data_path,patchsizes, feasible_information_patches, feasible_appartenance_patches,data_augmentation_type = 'None' ,suffixes=('lr', 'hr'), patch_suffixes = ('lr', 'hr') ,patched_shapes = {'naip':(40,40), 'spain_crops': (42,42), 'spain_urban': (42,42), 'spot': (42,42)}, P_null_path = '/localhome/iaga_dv/Dokumente/Operators/P_null_32.npy' ) :
        ...
        for img_file in self.img_file_list:
            ...
            # Number of the image in the subdataset
            nb_img = os.path.normpath(img_file).split(os.sep)[-2]
            # Subdataset of the image
    ...
    def get_Fy_fullimg_idx_V2(self, subdataset, img_idx_insubds, feasible_info,feas_app, lim_area_ratio = 0.6, n_iter_max_ratio = 0.4):
        ...
        while n_iter < nr*nc* n_iter_max_ratio and replaced_prop_lim1 < lim_area_ratio and replaced_prop_lim2 <lim_area_ratio : 
            ...
            img_lim1_id = lim_patch1_id[0], lim_patch1_id[1]   # The id of the image where P1 belongs (subdataset, img idx)
            subds_img_lim1 = img_lim1_id[0]
            patched_shape_lim1 = self.patched_shape[subds_img_lim1]
            patch_lim1_nb = lim_patch1_id[-1]    # Its place on the image, in the 1d array
...
class SRDataset_lightload(Dataset):
    def __init__(self, data_path,patchsizes, suffixes=('lr', 'hr') ,patched_shapes = {'naip':(40,40), 'spain_crops': (42,42), 'spain_urban': (42,42), 'spot': (42,42)} ) :
        """
        Args:
            folder_path (str): Path to folder containing image patches.
            suffixes (tuple): Suffixes used in filenames for LR, HR.
        ...
        #self.img_file_list = [os.path.basename(f).replace(f'_{self.suffix_list[0]}.tif', '') for f in self.img_file_list]
        self.patch_ids = []
        self.image_ids = []
        imgs_idx_list = {}
